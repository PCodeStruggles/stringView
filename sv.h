...
#define sv_at(sv, pos)                                                         \
  (sv).data[assert((pos) >= 0 && (pos) < (sv).count), (pos)]
...
// Return whether sv and cstr are equal
...
// Return the content of the sv passed as argument up to the delimeter passed as
// argument. Increment the count of the sv passed as argument in order to point
// to the next token.
...
sv sv_from_cstr(const char *cstr) {
  if (cstr == NULL)
    SV_PANIC("sv_from_cstr: cstr argument cannot be NULL");
...
bool sv_contains(const sv sv, const char *cstr) {
  if (cstr == NULL || strlen(cstr) > sv.count)
    return false;
  size_t cstr_len = strlen(cstr);
  size_t i = 0;
  while (i < sv.count) {
    for (size_t k = i, j = 0; j < cstr_len && sv.data[k] == cstr[j]; k++, j++) {
      if (j == (cstr_len - 1))
        return true;
    }
    i++;
...
bool sv_starts_with(const sv sv, const char *prefix) {
  if (prefix == NULL)
    SV_PANIC("sv_starts_with: prefix argument cannot be NULL");
  size_t prefix_len = strlen(prefix);
  size_t i = 0;
...
sv sv_trim_left(const sv sv) {
  if (sv.count <= 0)
    return (struct string_view){.data = "", .count = 0};
  size_t i = 0;
...
sv sv_trim_right(const sv sv) {
  ...
  return (struct string_view){.data = sv.data, .count = sv.count - i};
}
...
sv sv_split_by_delim(sv *string_view, const char c) {
  ...
  for (; string_view->count > 0 && isspace(string_view->data[0]);
       string_view->data += 1, string_view->count -= 1)
    ;

  size_t i = 0;
  while (i < string_view->count && string_view->data[i] != c)
    i++;
